      ...
      <div className="space-y-4">
        <Panel>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {platforms.map((p) => (
              <div key={p.name} className="neon-panel rounded-md p-3 hover:bg-neon/5 transition">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-4 w-full">
                ...
                <div className="flex items-end gap-1 h-20">
                  {monthData.map((v, i) => (
                    <motion.div key={i} className="flex-1 bg-neon/70 rounded-sm" initial={{ height: 0 }} animate={{ height: `${v / 3}px` }} transition={{ delay: i * 0.04 }} />
                  ))}
                </div>
              ...
              {langs.map((l) => (
                <div key={l.name} className="text-xs">
                  ...
                  <div className="h-1.5 bg-foreground/10 rounded-sm overflow-hidden">
                    <motion.div className="h-full bg-neon" initial={{ width: 0 }} animate={{ width: `${l.value * 2.5}%` }} transition={{ duration: 1 }} />
                  </div>
                </div>
              ))}
            </div>
